...
COPY --from=package-builder /app/dist/mapio_webserver_back*.whl /app/
RUN pip --python "$(which python)" install --no-cache-dir --no-deps ./mapio_webserver_back*.whl

ENTRYPOINT [ "mapio_webserver_back" ]
